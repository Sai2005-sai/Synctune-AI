...
export function assignTracksToSegments(
  ...
  const trackDur = getTrackDur(primaryTrack);
  
  // To ensure the video gets a proper "build and outro" instead of just a slow intro,
  // we offset the track so its natural ending aligns with the video's ending.
  // We leave a small 1-second buffer.
  const startOffset = Math.max(0, trackDur - videoDuration - 1);
