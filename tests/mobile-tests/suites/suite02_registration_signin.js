...
module.exports = async function suite02(driver, L) {
  ...
  const dashChecks = [
    ...
    [36, () => U.findTextContains(driver,     'Summer Vlog',         4000).then(e=>{if(!e)throw new Error('not found');}), '"Summer Vlog 2023" project card'],
    [37, () => U.findTextContains(driver,     'Cinematic Drone',     4000).then(e=>{if(!e)throw new Error('not found');}), '"Cinematic Drone" project card'],
    [38, () => U.findTextContains(driver,     'Workout Routine',     4000).then(e=>{if(!e)throw new Error('not found');}), '"Workout Routine" project card'],
    [39, () => U.findText(driver,             'Completed',           4000).then(e=>{if(!e)throw new Error('not found');}), '"Completed" status badge'],
